// core/user/user.controller.ts
import { api, APIError } from 'encore.dev/api'
import type { CreateUserDto, UpdateUserDto, Response, UserResponse } from './user.interface'
 ...
 */
export const create = api({ expose: true, method: 'POST', path: '/users' }, async (data: CreateUserDto): Promise<UserResponse> => {
	try {
		if (!data.email || !data.username || !data.password) {
			throw APIError.invalidArgument('Missing email, username or password')
		}
 ...
 * Get user data by id
 */
export const readOne = api({ expose: true, method: 'GET', path: '/users/:id' }, async ({ id }: { id: number }): Promise<UserResponse> => {
	try {
 ...
 * Delete user by id
 */
export const destroy = api({ expose: true, method: 'DELETE', path: '/users/:id' }, async ({ id }: { id: number }): Promise<Response> => {
	try {
